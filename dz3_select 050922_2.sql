...
join record_collection rc on c.c_id = rc.collection_id 
join records r on rc.record_id = r.r_id 
join singer_album sa on r.album_id = sa.album_id 
join singers s on sa.singer_id = s.s_id 
where s.singer_name = 'КИНО';
...
select record_name from records r
left join record_collection rc on r.r_id = rc.record_id 
...
join records r on r.album_id = a.a_id 
group by a.album_name 
having count(*) = (
select count(record_name)  from records r
join albums a on r.album_id = a.a_id 
group by album_name 
limit 1 offset 13);
